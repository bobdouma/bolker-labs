...
# Learning goals
This lab has two goals. First, to make you familiar with the technicalities of stochastic distributions in `R`. In particular, how to generate values from probability distributions and how to make your own probability distribution  (section 2-6). The second part helps you practicing with probability distributions on different types of data. Under time constraints, make sure that you make at least the exercises in section 2, 3 and 7. 
...
```{block2, type= "solutionbox", latex.options="{block-green}{4}",echo=T}
...
```{block2, type = "exercisebox",latex.options="{4}"}
Figure out what it means that `mean(tabdat)` equals `r mean(tabdat)`.
...
`segments(x0=exp(mean(log(rf))),y0=mean(log(rf)),x1=exp(mean(log(rf))),
          y1=min(log(rf)),lty=2)` 
