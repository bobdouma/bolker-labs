...
```{block2, type='solutionbox', latex.options="{block-green}{6}",echo=T}
...
## Likelihood surface
To find the likelihood surface follow the steps below (background information can be found in Bolker Ch. 6).
...
`mle2.1 = mle2(nll.mle,start=list(a=25,b=0.0080,sd=1),`

               `data=data.frame(x=shapes1$x,y=shapes1$y),method="Nelder-Mead")` 
...
`mle2.2 = mle2(nll.mle.alt,start=list(a=25,b=0.0080,sd=1),`
...
The second model fits better according to AIC. Let's plot both curves. 


`curve(coef(mle2.2)[1]*x/(coef(mle2.1)[2]+x)),add=T,col="red")`



You see that the difference is rather subtle. Yet, the difference is about three points on the log Likelihood scale. So that implies that the second model makes the data exp(3) $\approx$ 20 times more likely!
