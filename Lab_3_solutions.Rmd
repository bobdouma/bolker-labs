...
sigma.self = function(x,na.rm=F){
  ...
  n = length(ifelse(na.rm,na.omit(x),x))
...
A quadratic functionh $x^2$ has a maximum. or minimum. This maximum/minimum is at $x=0$. To shift the curve to have its maximum at $x=5$, you need to subtract $5$ from the $x$, i.e. $(x-5)^2$. The maximum of this function is at $y=0$, so one need to add $1$ to the function, i.e. $(x-5)^2+1$.
