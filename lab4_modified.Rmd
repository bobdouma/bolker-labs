...
```{r,echo=FALSE,fig.cap="Figure 1. `R` functions for an arbitrary distribution `dist`, showing density function (`ddist`), cumulative distribution function (`pdist`), quantile function (`qdist`), and random-deviate function (`rdist`).", fig.height = 3.5}
sh = 4
textsize = 1.2
...
```{r,fig=TRUE,echo=FALSE, fig.cap = "Figure 2. Results of `rbinom`"}
plot(factor(rbinom(200,size=12,prob=0.1)),xlab="# of successes",
     ylab="# of trials out of 200")
``` 
...
```{r,fig=TRUE,echo=FALSE, fig.cap = "Checking binomial deviates against theoretical values."}
x = rbinom(10000,prob=p,size=N)
tx = table(factor(x,levels=0:12))/10000
b1 = barplot(tx,ylim=c(0,0.23),ylab="Probability")
points(b1,dbinom(0:12,prob=p,size=N),pch=16)
...
and $b = 9.79$ (asymptote, recruits per 0.1 m$^2$ patch reef at high settlement density).s
