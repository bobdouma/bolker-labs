...
This lab has two parts. A part to make you familiar with the technicalities of stochastic distributions in `R`: how to generate values from probability distributions; how to make your own probability distribution etc (section 1-5), and a part (section 6) to help you practicing with probability distributions on different types of data.  Under time constraints, make sure that you make at least the exercise in section 1 and section 6. 
...
These four functions exist for each of the distributions `R` has built in: e.g. for the normal distribution they're `rnorm()`, `pnorm()`, `dnorm()`, `qnorm()`. Each distribution has its own set of parameters (so e.g. `pnorm()` is `pnorm(x,mean=0,sd=1)`). To see which distributions are available in the `base` package; check `?distributions`. See Figure 2.
...
with a gamma distribution instead of an uniform distribution.
...
and vice versa (use pen and paper). Explain why you might prefer Morris's parameterization. Define a new set of functions that generate random numbers from the beta distribution (`my_rbeta`) and calculate the density function (`my_dbeta`) in terms of $P$ and $\theta$. Generate a histogram from this distribution
...
**Exercise 9** Reload the six datasets and choose for each dataset one or two candidate probability distributions.
